...
export interface ToolData {
  ...
  mono: string;
  color: string;
  /** Optional dark-mode override for `color`, used when the light-mode hex
   * reads too muddy against the dark card/page background (e.g. GitHub's
   * near-black #333333). See the 2026-02 accessibility pass notes. */
  colorDark?: string;
}

export default function ToolIcon({ name, mono, color, colorDark }: ToolData) {
  const color_ = resolveAccent(color, colorDark, useIsDarkTheme());
  return (
    <div className="group flex flex-col items-center gap-2.5">
      <div className="relative flex h-16 w-16 items-center justify-center transition-transform duration-200 ease-out will-change-transform group-hover:scale-[1.03]">
        <div
          ...
          style={{ background: `conic-gradient(from -45deg, ${color_}70, transparent 62%)` }}
        />
